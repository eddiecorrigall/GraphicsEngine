    ...
    for (int i = 0; i < header.numberOfTriangles; i++) { // For each triangle...
        for (unsigned char j = 0; j < 3; j++) { // For each triangle vertex...
            ...
            if (gfx->IsDebuggingLighting()) {
                ...
                // Draw a line gradient from the surface (blue) to the light position (red)...
                ...
                glColor3f(0.0f, 0.0f, 1.0f); // Blue at source
                glVertex3fv(glm::value_ptr(source));

                glColor3f(1.0f, 0.0f, 0.0f); // Red at destination
            ...
            if (gfx->IsDebuggingView()) {
                ...
                // Draw a line gradient from the surface (blue)
                // perpendicular (so it is visible) to the camera position (red)...
            ...
            if (gfx->IsDebuggingNormals()) {
                ...
                // Debug normals...
                // Draw a line gradient from the surface (blue) along the surface normal (red)...
